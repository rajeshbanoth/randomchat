...
function startSearch(socketId, options = {}) {
  try {
    ...
    matchingEngine.addUser(socketId, {
      userId: socketId,
      profile: userWrapper.profile,
      chatMode: userWrapper.profile.chatMode || "text",
      timestamp: Date.now(),
    });
...
function instantMatch(userId1, userId2, compatibility = 50, matchMode = "text") {
  try {
    ...
    if (matchMode === "video") {
      console.log(`[instantMatch] Video match between ${userId1} and ${userId2}`);
      ...
      console.info(`[instantMatch] Video call setup: ${callId} for room ${roomId}`);
    }
...
function handleWebRTCOffer(socket, data) {
  try {
    ...
    console.log(`[webrtc-offer] ${from} -> ${to}, callId: ${providedCallId}`);
...
function handleWebRTCAnswer(socket, data) {
  try {
    ...
    console.log(`[webrtc-answer] ${from} -> ${to}, callId: ${callId}`);
    ...
    if (!callUpdated) {
      console.warn(`[webrtc-answer] Call ${callId} not found`);
    }
...
function handleWebRTCIceCandidate(socket, data) {
  try {
    ...
    const partner = activeUsers.get(to);
    if (!partner) {
      console.warn(`[webrtc-ice-candidate] Partner ${to} not found`);
      return;
    }
...
function handleWebRTCEnd(socket, data) {
  try {
    ...
    console.log(`[webrtc-end] Cleared call records for ${from} and ${to}`);
  } catch (err) {
    console.error("[webrtc-end] error:", err);
  }
}
  ...
  try {
    ...
    socket.on("register", (userData) => {
      try {
        ...
        console.error("[register socket] error:", e);
      }
    });
    ...
    socket.on("webrtc-ice-candidate", (data) => handleWebRTCIceCandidate(socket, data));
    socket.on("webrtc-end", (data) => handleWebRTCEnd(socket, data));
    socket.on("webrtc-reject", (data) => handleWebRTCReject(socket, data));
    socket.on("video-call-status", (data) => handleVideoCallStatus(socket, data));
    socket.on("call-toggle-media", (data) => handleCallToggleMedia(socket, data));
    socket.on("screen-share-status", (data) => handleScreenShareStatus(socket, data));
...
server.listen(PORT, () => {
  console.info(`Server running on port ${PORT}`);
  console.info(`Health: http://localhost:${PORT}/health`);
  console.info(`WebRTC Config: http://localhost:${PORT}/webrtc/config`);
});
